...
func synchronize2(reportOnly bool) (log string) {
	...
	for _, c := range client_updated_contexts {
		...
		*/
		...
		var tid int
		err := pdb.QueryRow("SELECT tid from context WHERE tid=$1", c.tid).Scan(&tid)
		err = row.Scan(&tid)
		switch {
		...
		case err == sql.ErrNoRows:
			...
			*/
			err = pdb.QueryRow("INSERT INTO context (title, star, created, modified, deleted) VALUES (MAX(tid)+1, $1, $2, $3, now(), false) RETURNING tid;",
				c.title, c.star, c.created).Scan(&tid)
			if err != nil {
				fmt.Fprintf(&lg, "Error inserting new context %q into postgres: %v", truncate(c.title, 15), err)
				break
			}
		...
		default:
			...
			if err != nil {
				fmt.Fprintf(&lg, "Error updating postgres for context %q with tid %d: %v\n", truncate(c.title, 15), c.tid, err)
			} else {
				fmt.Fprintf(&lg, "Updated server/postgres context: *%q* with tid: **%d**\n", c.title, c.tid)
			}
		}
	}
