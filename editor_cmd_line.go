...
func (e *Editor) quitActions() {
	...
	index := -1
	for i, w := range appCtx.Windows {
		if w == e {
			index = i
			break
		}
	}
	copy(appCtx.Windows[index:], appCtx.Windows[index+1:])
	appCtx.Windows = appCtx.Windows[:len(appCtx.Windows)-1]
